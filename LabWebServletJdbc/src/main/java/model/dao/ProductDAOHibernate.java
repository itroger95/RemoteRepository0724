...
public class ProductDAOHibernate implements ProductDAO {
	...
	public ProductBean insert(ProductBean bean) {
		if(bean!=null) {
			ProductBean result = (ProductBean) this.getSession().get(ProductBean.class, bean.getId());
			if(result==null) {
	...
	public ProductBean update(String name, double price, Date make, int expire, int id) {
		ProductBean result = (ProductBean) this.getSession().get(ProductBean.class, id);
		if(result!=null) {
			result.setName(name);
